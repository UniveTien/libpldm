...
#define PLDM_FWUP_PACKAGE_HEADER_INFO_V130_MIN_SIZE 46
...
struct pldm_firmware_device_id_record_v130 {
	...
	const void* comp_image_set_version_string;
	const void* fw_device_pkg_data;
...
#define foreach_pldm_firmware_device_id_record_v130(info, recs, rec, rc)		\
	for ((rc) = 0; (!pldm_firmware_device_id_record_v130_iter_end(&(recs)) && \
			!((rc) = decode_pldm_firmware_device_id_record_v130( \
				  &(info), &(recs), &(rec))));                  \
...
#define foreach_pldm_downstream_device_id_record_v130(info, recs, rec, rc)		\
	for ((rc) = 0; (!pldm_downstream_device_id_record_v130_iter_end(&(recs)) && \
			!((rc) = decode_pldm_downstream_device_id_record_v130( \
				  &(info), &(recs), &(rec))));                  \
...
#define foreach_pldm_component_image_information_v130(infos, info, rc)		\
	for ((rc) = 0; (!pldm_component_image_information_v130_iter_end(&(infos)) && \
			!((rc) = decode_pldm_component_image_information_v130(   \
				  &(infos), &(info))));                          \
...
#define foreach_pldm_descriptor(descs, desc, rc) \
	for ((rc) = 0; (!pldm_descriptor_iter_end(&(decs)) && \
		!((rc) = decode_pldm_descriptor_from_iter(&(decs), &(desc)))); \
	     pldm_descriptor_iter_next(&(decs)))
...
#define foreach_pldm_downstream_device_descriptor(devs, dev, desc, rc)         \
	struct pldm_descriptor_iter desc##_iter =                         \
		pldm_downstream_device_descriptor_iter_init(            \
			&(devs), &(dev));                                   \
	foreach_pldm_descriptor(desc##_iter, desc, rc)
// #define foreach_pldm_downstream_device_descriptor(devs, dev, desc, rc)         \
...
int decode_firmware_device_id_record_from_iter(
	const uint8_t *data, size_t length,
	struct pldm_firmware_device_id_record_v130_iter *iter,
	struct pldm_firmware_device_id_record_v130 *fw_device_id_record,
	struct pldm_descriptor_iter *record_descriptors);

/** @brief Decode individual downstream device ID record from iterator
 *
 *  @param[in] data - pointer to downstream device ID record
 *  @param[in] length - available length in the firmware update package
 ...
 */
int decode_pldm_comp_image_info_from_iter(
	const uint8_t *data, size_t length,
	struct pldm_component_image_infornation_v130_iter *iter,
	struct pldm_component_image_information_v130
		*component_image_info);
